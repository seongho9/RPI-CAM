...
video::VideoStreamerGST::VideoStreamerGST() : _video_config(){
    rtsp_server = nullptr;
    mountPoint = nullptr;
    mfactory = nullptr;
    main_loop = nullptr;
...
video::VideoStreamerGST::~VideoStreamerGST() {
    // 마운트 포인트 객체 참조 해제
...
void video::VideoStreamerGST::make_server(){
        // RTSP 서버 생성
...
std::string video::VideoStreamerGST::createPipeline(const config::VideoConfig& config) {
    
    //pipeline 구성 
    //config 파일에서 직접 가지고 와 사용해야
    std::stringstream pipeline_stream;
    pipeline_stream << "( libcamerasrc ! "
                    << "video/x-raw,width=" << config.width() 
                    << ",height=" << config.height() 
                    << ",framerate=" << config.frame_rate()
                    << ",format=" << config.format()   // format으로 수정 필요
                    << " ! tee name=t ! "
...
int video::VideoStreamerGST::start_server(){
...
int video::VideoStreamerGST::stop_server(){
    // 메인 루프 종료
    if (main_loop) {
